...
// Get category
$result = $db->query('SELECT * FROM ' . NV_PREFIXLANG . '_' . $module_data . '_categories WHERE id = ' . $id);
if (!$result->rowCount()) {
    nv_redirect_location(NV_BASE_ADMINURL . 'index.php?' . NV_LANG_VARIABLE . '=' . NV_LANG_DATA . '&' . NV_NAME_VARIABLE . '=' . $module_name . '&op=categories');
}
$row = $result->fetch();
...
if ($nv_Request->isset_request('submit', 'post')) {
    ...
    // Update database
    $stmt = $db->prepare('UPDATE ' . NV_PREFIXLANG . '_' . $module_data . '_categories SET title = :title, description = :description, weight = :weight WHERE id = :id');
